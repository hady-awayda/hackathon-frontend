  ...
  const PaidFeatureDashboard = () => {
    ...
    useEffect(() => {
      ...
      const storedResults = JSON.parse(localStorage.getItem(`paidUserResults_${userEmail}`)); 
      if (storedResults) {
    ...
    const handleSaveIdea = () => {
      ...
      fetch(apiUrl, {
        ...
        .then(data => {
          ...
          localStorage.setItem(`paidUserResults_${userEmail}`, JSON.stringify(updatedResults));
          setShowModal(false);
        })
        .catch((error) => {
    ...
    const handleDetailsClick = () => {
      navigate('/streamlit', { state: { data: results } });
    };
    ...
    return (
      <div className="idea-list h-screen bg">
        ...
        {results.length > 0 && results.map((result, index) => (
          <div key={index} className="result mt-8 p-6 bg-gray-800 text-white rounded-lg shadow-lg">
            <h3 className="text-2xl font-semibold mb-4">Similar Apps (Response {index + 1}):</h3>
            {result.similar_apps && result.similar_apps.length > 0 ? (
              <ul className="list-disc list-inside mb-6">
                {result.similar_apps.map((app, appIndex) => (
                  <li key={appIndex}>
